...
function(initCppJsLib target source_dir include_dir)
    ...
    if (HTTPS)
        ...
        if (WIN32)
            ...
            if (OPENSSL_FOUND)
                ...
                target_link_directories(${target} PRIVATE "${OPENSSL_INCLUDE_DIR}/../lib")
                add_compile_definitions(CPPJSLIB_ENABLE_HTTPS)
            else ()
    ...
    if (DEFINED BOOST_LIBRARY_DIRS OR (DEFINED OPENSSL_SSL_LIBRARY AND DEFINED OPENSSL_CRYPTO_LIBRARY))
        target_link_directories(${target} PUBLIC ${BOOST_LIBRARY_DIRS})
        # On darwin: cannot link against *.dylib
        if (NOT APPLE)
            target_link_directories(${target} PUBLIC ${OPENSSL_SSL_LIBRARY} ${OPENSSL_CRYPTO_LIBRARY})
        endif ()
    else ()
    ...
    if (HTTPS)
        if (WIN32)
            ...
            find_library(LIBCRYPTO crypto HINTS ${OPENSSL_CRYPTO_LIBRARY})
